...
@section('content')
    ...
    <form action="{{ route('admin.step.update',$step->id) }}" method="post">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="product_id" id="product_id">
                @foreach($products as $product)
                    <option value="{{ $product->id }}" @if($product->id == $step->product_id) selected @endif>{{ $product->name  }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="en">英语</label>
            <script id="en" name="en" type="text/plain">
                {!! $step->en !!}
            </script>
        </div>
        <div class="form-group">
            <label for="de">德语</label>
            <script id="de" name="de" type="text/plain">
                {!! $step->de !!}
            </script>
        </div>
        <div class="form-group">
            <label for="fr">法语</label>
            <script id="fr" name="fr" type="text/plain">
                {!! $step->fr !!}
            </script>
        </div>
        <div class="form-group">
            <label for="it">意大利语</label>
            <script id="it" name="it" type="text/plain">
                {!! $step->it !!}
            </script>
        </div>
        <div class="form-group">
            <label for="es">西班牙语</label>
            <script id="es" name="es" type="text/plain">
                {!! $step->es !!}
            </script>
        </div>
        <div class="form-group">
            <label for="hu">匈牙利语</label>
            <script id="hu" name="hu" type="text/plain">
                {!! $step->hu !!}
            </script>
        </div>
        <div class="form-group">
            <label for="ko">韩语</label>
            <script id="ko" name="ko" type="text/plain">
                {!! $step->ko !!}
            </script>
        </div>
        <div class="form-group">
            <label for="ja">日语</label>
            <script id="ja" name="ja" type="text/plain">
                {!! $step->ja !!}
            </script>
        </div>
        <div class="form-group">
            <label for="cn">简体中文</label>
            <script id="cn" name="cn" type="text/plain">
                {!! $step->cn !!}
            </script>
        </div>
        <div class="form-group">
            <label for="tw">繁体中文</label>
            <script id="tw" name="tw" type="text/plain">
                {!! $step->tw !!}
            </script>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="sort" name="sort" value="{{ $step->sort }}" required>
        </div>
